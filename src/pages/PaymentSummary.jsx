...
export default function PaymentSummary() {
  ...
  const generateABID = () => {
    const base = Math.random().toString(36).substring(2, 7); // generates 5-char alphanumeric
    return `ABID-${base.toUpperCase()}`; // e.g. ABID-ABC12
  };



  const createTicket = async (e, uid, evid,uname) => {
    e.preventDefault();
    //!adding a ticket qr code to booking ----------------------
    try {
      const abid = generateABID();
      ...
      // ading user in event database
      await axios.post('/addUser', { uid, evid,uname });
  ...
  async function generateQRCode(name, eventName,ab) {
    try {
      ...
      <div className="ml-12 bg-gray-100 shadow-lg mt-8 p-16 w-3/5 float-left">
        ...
        <div className="mt-10 space-y-4">
          ...
          <div className="float-right">
            ...
            <Link to={'/'}>
              <button type="button"
                onClick={(e) => createTicket(e, user._id, event._id,user.name)}
                className="primary">
